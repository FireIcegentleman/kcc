...
static const char zHelp[] =
    ...
    "  --heap SZ MIN       Memory allocator uses SZ bytes & min allocation "
    "MIN\n"
    ...
    "  --pcache N SZ       Configure N pages of pagecache each of size SZ "
    "bytes\n"
    "  --primarykey        Use PRIMARY KEY instead of UNIQUE where "
    "appropriate\n"
    "  --repeat N          Repeat each SELECT N times (default: 1)\n"
    "  --reprepare         Reprepare each statement upon every invocation\n"
    "  --serialized        Set serialized threading mode\n"
    "  --singlethread      Set single-threaded mode - disables all mutexing\n"
    "  --sqlonly           No-op.  Only show the SQL that would have been "
    "run.\n"
    "  --shrink-memory     Invoke sqlite3_db_release_memory() frequently.\n"
    "  --size N            Relative test size.  Default=100\n"
    "  --stats             Show statistics at the end\n"
    "  --temp N            N from 0 to 9.  0: no temp table. 9: all temp "
    "tables\n"
 ...
 */
static const char *isTemp(int N) { return g.eTemp >= N ? " TEMP" : ""; }
...
static void groupStep(sqlite3_context *context, int argc,
                      sqlite3_value **argv) {
  const char *zVal;
...
void testset_main(void) {
...
#if SQLITE_VERSION_NUMBER < 3005004
  sqlite3_create_function(g.db, "group_concat", 1, SQLITE_UTF8, 0, 0, groupStep,
                          groupFinal);
#endif
  ...
  speedtest1_exec("BEGIN");
  speedtest1_prepare("UPDATE t2 SET d=b*3 WHERE a=?1; -- %d times", n);
  for (i = 1; i <= n; i++) {
  ...
  speedtest1_exec("BEGIN");
  speedtest1_prepare("DELETE FROM t2 WHERE b BETWEEN ?1 AND ?2; -- %d times",
                     n);
  for (i = 1; i <= n; i++) {
  ...
  speedtest1_exec("BEGIN");
  speedtest1_exec("CREATE%s TABLE t5(a INTEGER PRIMARY KEY, b %s);", isTemp(9),
                  g.zNN);
  ...
  speedtest1_exec("BEGIN");
  speedtest1_exec(
      "CREATE%s TABLE t6(a TEXT PRIMARY KEY, b %s)%s;", isTemp(9), g.zNN,
...
void testset_cte(void) {
  ...
  speedtest1_prepare(
      "WITH RECURSIVE \n"
      "  xaxis(x) AS (VALUES(-2.0) UNION ALL SELECT x+?1 FROM xaxis WHERE "
      "x<1.2),\n"
      "  yaxis(y) AS (VALUES(-1.0) UNION ALL SELECT y+?2 FROM yaxis WHERE "
      "y<1.0),\n"
      "  m(iter, cx, cy, x, y) AS (\n"
...
void testset_fp(void) {
  ...
  speedtest1_exec("BEGIN");
  speedtest1_exec("CREATE%s TABLE t1(a REAL %s, b REAL %s);", isTemp(1), g.zNN,
                  g.zNN);
...
*/
static int xsliceGeometryCallback(sqlite3_rtree_geometry *p, int nCoord,
...
void testset_rtree(int p1, int p2) {
  ...
  if (g.bVerify) {
    n = g.szTest * 200;
    speedtest1_begin_test(111,
                          "Verify result from 1-D intersect slice queries");
...
void testset_orm(void) {
  ...
  speedtest1_prepare(
      ...
      "ZPOSTPROTEINHANDLEACTOR,ZLACEADDRESSGROUNDCAREFUL,"
      "ZIMAGEPENCILOTHERBOTTOM,"
      ...
      "ZSQUAREGLEEFULCHILDLIGHT,ZONERELEASEAVERAGENURSE,"
      "ZBIGTHINKCONVERTECONOMY,"
      ...
      "ZREWARDINSIDEMANGOINTENSE,ZCADETBRIGHTPLANETBANK,"
      "ZPLANETFAMILYPUREMEMORY,"
      ...
      "ZCHARGECLICKHUMANEHIRE,ZHERRINGJOKEFEATUREHOPEFUL,"
      "ZYARDOREGANOVIVIDJEWEL,"
      ...
      "VALUES(?1,?26,?20,?93,?8,?33,?3,?81,?28,?60,?18,?47,?109,?29,?30,?104,?"
      "86,"
      "?54,?92,?117,?9,?58,?97,?61,?119,?73,?107,?120,?80,?99,?31,?96,?85,?50,?"
      "71,"
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; zType[j]; j++) {
      switch (zType[j]) {
        ...
        case 'V':
        case 'B':
          sqlite3_bind_text64(g.pStmt, j + 2, zNum, len, SQLITE_STATIC,
                              SQLITE_UTF8);
          break;
      }
    }
  ...
  speedtest1_prepare(
      ...
      "ZPOSTPROTEINHANDLEACTOR,ZHERRINGJOKEFEATUREHOPEFUL,"
      "ZCABBAGESOCKEASEMINUTE,"
      "ZMUFFINDRYERDRAWFORTUNE,ZPROBLEMCLUBPOPOVERJELLY,"
      "ZGIVEVIVIDDIVINEMEANING,"
...
int main(int argc, char **argv) {
  ...
  if (nLook > 0 && szLook > 0) {
    ...
    rc = sqlite3_db_config(g.db, SQLITE_DBCONFIG_LOOKASIDE, pLook, szLook,
                           nLook);
  ...
  ** have been finalized */
  ...
  if (showStats) {
    ...
    sqlite3_db_status(g.db, SQLITE_DBSTATUS_LOOKASIDE_MISS_SIZE, &iCur, &iHi,
                      0);
    printf("-- Lookaside size faults:       %d\n", iHi);
    sqlite3_db_status(g.db, SQLITE_DBSTATUS_LOOKASIDE_MISS_FULL, &iCur, &iHi,
                      0);
